...
export default function TaskDetailConponent({ task, onNext, onBack }: Props) {
  ...
  return (
    <div className="mt-4 space-y-3 text-white">
      {onBack && <Button text="Повернутись до списку задач" onClick={onBack} />}

      <Typography mb>{task.title}</Typography>
      ...
      <AnimatePresence>
        {task.examples && task.examples.length > 0 && (
          <motion.div
            ...
            className="mt-2"
          >
            <Typography tag="h3">Приклади:</Typography>
      ...
      <div className="grid lg:grid-cols-[30%_70%] gap-4">
        <div className="bg-[#1e1e1e] flex flex-col justify-between border border-white/20 rounded-[20px] text-gray-100 whitespace-pre-wrap p-4">
          <Typography tag="p" color="ivory" mb>
            {task.description}
          </Typography>
      ...
      >
        <Button onClick={handleRun} color="amber" text="Запустити" />
